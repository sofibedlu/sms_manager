...
class ListSms(models.Model):
    ...
    def _check_content_length(self):
        max_length = 335  # Define your maximum character limit here
        ...
        for record in self:
            ...
            if content_length > max_length:
                raise ValidationError("The content exceeds the maximum length of {} characters. Current length is {} characters.".format(max_length, content_length))

    phone_numbers = fields.Char(string='Recipent Numbers', compute='_compute_phone_numbers_summary', store=True)

    @api.depends('contacts', 'number_lines', 'mail')
    def _compute_phone_numbers_summary(self):
        for record in self:
            numbers = []
            if record.type == 'list':
                numbers = [contact.mobile or contact.phone for contact in record.contacts if contact.mobile or contact.phone]
            elif record.type == 'number':
                numbers = [line.number for line in record.number_lines]
            elif record.type == 'mail':
                for mailing_list in record.mail:
                    if mailing_list.list_type == 'contacts':
                        numbers.extend([contact.mobile or contact.phone for contact in mailing_list.contacts if contact.mobile or contact.phone])
                    elif mailing_list.list_type == 'numbers':
                        numbers.extend([number_line.number for number_line in mailing_list.numbers])
            
            if len(numbers) > 1:
                record.phone_numbers = ', '.join(numbers[:1]) + ', ...'
            else:
                record.phone_numbers = numbers[0] if numbers else ''

    contact_name = fields.Char(string='Contact Name', compute='_compute_contact_name', store=True)

    @api.depends('contacts', 'type')
    def _compute_contact_name(self):
        for record in self:
            if record.type == 'list':
                if record.contacts:
                    record.contact_name = record.contacts[0].name
                    ...
                    if record.mail:
                        record.contact_name = record.mail[0].name
                        if len(record.mail) > 1:
                            record.contact_name += ', ...'
            else:
                record.contact_name = ''
    ...
    def create(self, vals):
        self._check_fields_filled(vals)
    ...
    def write(self, values):
        ...
        self._check_fields_filled(values)
        return super(ListSms, self).write(values)
    
    def _check_fields_filled(self, vals):
        # Determine the type either from vals or the existing record
        record_type = vals.get('type', self.type if self else None)
        if record_type == 'list' and not vals.get('contacts', self.contacts if self else None):
            raise ValidationError("Please add at least one contact.")
        elif record_type == 'number' and not vals.get('number_lines', self.number_lines if self else None):
            raise ValidationError("Please add at least one number.")
        elif record_type == 'mail' and not vals.get('mail', self.mail if self else None):
            raise ValidationError("Please select at least one mailing list.")
...
def send_same_message_to_many(uname, pwd, fr, message, numlist, url):
    ...
    if parsed_url.scheme == "http":
        ...
        #print(temp_url)
        temp_url = f"http://{parsed_url.hostname}:8080/secure/sendbatch"
        u = temp_url
    else:
        #urll = url
        temp_url = f"http://{url}:8080/secure/sendbatch"
        print(temp_url)
        u = temp_url
